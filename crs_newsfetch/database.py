...
class DatabaseManager:
    ...
    def insert_publication(self, author_id, title, year, url, form_submitted=0):
        try:
            self.cursor.execute('''
            INSERT INTO publications (author_id, title, year, url, form_submitted)
            VALUES (?, ?, ?, ?, ?)
            ''', (author_id, title, year, url, form_submitted))
            self.conn.commit()
        except Exception as e:
    ...
    def get_all_publications(self):
        ...
        self.cursor.execute('''
            SELECT publications.id, authors.name, publications.title, publications.year, publications.url, publications.form_submitted
            FROM publications
